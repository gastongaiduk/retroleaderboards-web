...
function buildRivalryFriend(
  ...
  const friendMap = new Map(
    friendLeaderboards.filter((lb) => lb.UserEntry).map((lb) => [lb.ID, lb]),
  );
...
export const useRivalsStore = defineStore("rivals", () => {
  ...
  const loadRivalries = async (): Promise<void> => {
    ...
    rivalryGames.value.sort((a, b) => {
      const totalA = a.rivals.reduce(
        (sum, r) => sum + r.wins.length + r.losses.length,
        0,
      );
      const totalB = b.rivals.reduce(
        (sum, r) => sum + r.wins.length + r.losses.length,
        0,
      );
